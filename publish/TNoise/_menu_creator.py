...
try:
    ...
    from TNoise._plugin_loader import ensure_node_class_loaded
except Exception:
    from _consts import (
        ICON_FILENAME,
        MENU_NAME,
...
def create_node():
    ...
    try:
        ...
        nuke.tprint("[TNoise] Unable to create node '{}': {}".format(NODE_CLASS_NAME, error))
...
def add_menu():
    ...
    if os.getenv(PLUGIN_LOADED_ENV_VAR) != "1":
        logger.warning("TNoise menu created, but plugin binary is not loaded yet.")
