...
try:
    from TNoise._consts import NODE_CLASS_NAME
except Exception:
...
class TNoiseInlineRampKnob:
    ...
    def makeUI(self):
        ...
        if node is None or node.knob(_SERIALIZED_KNOB) is None:
            return QtWidgets.QLabel("Color ramp node context not available.")
...
def open_python_ramp_editor(node=None):
    ...
    if node is None or node.knob(_SERIALIZED_KNOB) is None:
        nuke.message("Select a node with a color ramp first.")
        return
